...
write.csv(travel_tips, "travel_tips.csv")
```
...
bali_desc <- data.frame(bali_place,bali_place_desc)
```
...
yogya_desc <- data.frame(yogya_place,yogya_place_desc)
```
...
mksr_desc <- data.frame(mksr_place,mksr_place_desc)
```
...
mlg_desc <- data.frame(mlg_place,mlg_place_desc)
```
...
bdg_desc <- data.frame(bdg_place,bdg_place_desc)
```
...
#latitude <- -8.518823° N
#longitude <- 115.258385° E
```

```{r}
#Kuta

#3.5 star (tripadivisor)
...
#latitude : -5.134066199999999
#longitude : 119.4046799
```
...
#latitude : -7.888957500000001
#longitude : 112.5296176
```
...
#latitude : -7.166203899999999
#longitude : 107.4021256
```
...
write.csv(review,"review.csv") #dont run again, the complete csv already in github :)
